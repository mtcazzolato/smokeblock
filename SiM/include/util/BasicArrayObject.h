    ...
    public:
        ...
        */
        void resize(size_t size, DType value){
            ...
            std::vector<DType> aux;
            for (size_t x = 0; x < min(size, data.size()); x++){
                aux.push_back(data[x]);
            }
            data.clear();
            aux.clear();
            data = aux;
            for (size_t x = min(size, data.size()); x < size; x++){
        ...
        */
        u_int32_t getSerializedSize(){
            ...
            return (sizeof(u_int32_t) + sizeof(size_t) +  (sizeof(DType) * data.size()) + 1);
        }
        ...
        */
        void unserialize(const u_char *dataIn, size_t dataSize){
